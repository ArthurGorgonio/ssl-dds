...
plot_path = list(set([i.split('/')[1].split('_')[0].lower()
                 for i in files])) * len(set(batches))
plot_path.sort()
...
for metric, title in zip(metric_names, titles):
    match = [data for data in files if metric in data]

    for data, dots, batch, path in zip(match, drift, batches, plot_path):
        plot_name = title + ' - Batches ' + batch
        fig_name = path + '/' + plot_name
        print('Data: ', data, '\tDrift: ', dots, '\nFigName: ', fig_name)
        os.system("gnuplot -c plot.gnuplot '{}' '{}' 'Valor' '{}' '{}' '{}' '{}'".format(
            data, dots, plot_name + 'Best Fit', fig_name + 'best-fit.png', 0, 1))
        os.system("gnuplot -c plot.gnuplot '{}' '{}' 'Valor' '{}' '{}' '{}' '{}'".format(data, dots,
                  plot_name + 'Best Fit - Não Agregado', fig_name + 'not-aggregated-best-fit.png', 0, 2))
