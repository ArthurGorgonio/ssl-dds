...
for (i in 1:3) {
  img_name <- paste(str_sub(strsplit(csv, '.', T)[[1]][1], 7), metric[i],
                    '.png', sep='')
...
png(paste(path, str_sub(strsplit(csv, '.', T)[[1]][1], 7), 'drifts', '.png',
    sep=''), 1280, 720)
...
png(paste(path, str_sub(strsplit(csv, '.', T)[[1]][1], 7), 'drifts-suavizado-',
    '.png', sep='') , 1280, 720)
