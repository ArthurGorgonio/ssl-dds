...
base_name = ['DyDaSL N - ', 'DyDaSL NFT - ',
             'DyDaSL W - ', 'DyDaSL H - ', 'DyDaSL C - ']
base_name.sort()
base_batch = [100, 250, 500, 750, 1000, 2500, 5000]
drift_names = ['Drift – N', 'Drift – NFT',
               'Drift – W', 'Drift – H', 'Drift – C']
drift_names.sort()
...
header = ['CPSSDS', 'Hinkley', 'DyDaSL N', 'DyDaSL FT', 'DyDaSL W',
          'Mean C', 'Mean H', 'Mean N', 'Mean FT', 'Mean W']
...
def cumulative_sum(path: str, values: list, column_names: list):
    ...
    data['Drift – C'] = np.where(data['Drift – C'], 1, 0)
    ...
    data['W Frequency'] = data['Drift – W'].cumsum(
    ) / (sum(data['Drift – W']) if sum(data['Drift – W']) > 0 else 1)
